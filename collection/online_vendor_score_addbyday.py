...
def get_vendor_score():
    sql = """SELECT t.vendor_id,
            ...
            FROM (select u.line_item_id, w.vendor_id,
                    ...
                    and u.status=1
                    and w.vendor_id in (select vendor_id
                        from db.t_vendor_ext
                        where type=4 and order_blocked=0 and oppo_offline=0)
                    group by 1
                    having DPB>0
                    )t
            GROUP BY 1"""
...
def count_all_lineitemid():
    sql = """select vendor_id, count(*) as count_all
            from db.t_line_item
            where (sales_status = 6 or sales_status = 7)
...
def merge_df():
    ...
    conn = connect_db()
    df.to_sql(name='r_online_vendor_score', con=conn, chunksize=20, if_exists='append', index=False)
    conn.close()


if __name__ == '__main__':
    merge_df()
    print 'you got the online vendor score already'
